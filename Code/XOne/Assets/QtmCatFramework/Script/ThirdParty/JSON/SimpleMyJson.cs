...
namespace QtmCatFramework 
{
	...
	public class SimpleMyJson 
	{
		...
		internal static IJsonNode ScanFirst(char c) 
		{
			...
			{
				return new JsonNodeValueString();
			} 
			else 
			{
				return new JsonNodeValueNumber();
			}
		}
		...
		public interface IJsonNode 
		{
			...
			void Scan(SimpleMyJson.ScanObj scan);
		...
		public class JsonNodeValueNumber : IJsonNode 
		{
			public JsonNodeValueNumber() 
			{

			}

			public JsonNodeValueNumber(double value)
			{
				this.value = value;
				this.isBool = false;
			}

			public JsonNodeValueNumber(bool value) 
			{
			...
			public void Scan(SimpleMyJson.ScanObj scan) 
			{
				string number = "";
			...
			public static implicit operator double(JsonNodeValueNumber m) 
			{
				return m.value;
			}

			public static implicit operator float(JsonNodeValueNumber m) 
			{
				return (float)m.value;
			}

			public static implicit operator int(JsonNodeValueNumber m)
			{
				return (int)m.value;
			}

			public static implicit operator uint(JsonNodeValueNumber m) 
			{
				return (uint)m.value;
			}

			public static implicit operator bool(JsonNodeValueNumber m) {
		...
		public class JsonNodeValueString : IJsonNode 
		{
			public JsonNodeValueString()
			{

			}
			public JsonNodeValueString(string value)
			{
				this.value = value;
			}
			...
			public void Scan(SimpleMyJson.ScanObj scan)
			{
				...
				if(unicode) 
				{
					MatchCollection reg = Regex.Matches(_value, @"\\u\w{4}");
					for(int i = 0; i < reg.Count; i++) 
					{
						_value = _value.Replace(reg[i].Groups[0].Value, "" + Regex.Unescape(reg[i].Value.ToString()).ToString());
					}
				}
			...
			public static implicit operator string(JsonNodeValueString m) 
			{
				return m.value;
			}
			...
			{
				int res;
				if(int.TryParse(this.value, out res))
				{
					return res;
				}
				else 
				{
					throw new FormatException(value + " can't parse to int!");
				}
			}
		...
		public class JsonNodeArray : List<IJsonNode>, IJsonNode 
		{
			...
			public void Scan(SimpleMyJson.ScanObj scan) 
			{
				for(int i = scan.seed + 1; i < scan.json.Length; i++) 
				{
					...
					IJsonNode node = SimpleMyJson.ScanFirst(c);
			...
			public void AddArrayValue(double value) 
			{
				this.Add(new SimpleMyJson.JsonNodeValueNumber(value));
			}
			...
			public void AddArrayValue(string value)
			{
				this.Add(new SimpleMyJson.JsonNodeValueString(value));
			}
			...
			public void SetArrayValue(int index, double value)
			{
				this[index] = new SimpleMyJson.JsonNodeValueNumber(value);
			}
			...
			public void SetArrayValue(int index, string value)
			{
				this[index] = new SimpleMyJson.JsonNodeValueString(value);
			}
		...
		public class JsonNodeObject : Dictionary<string, IJsonNode>, IJsonNode 
		{
			...
			public T Get<T>(string path, T defaultV) where T : IConvertible
			{
				if(defaultV == null) 
				{
					throw new ArgumentException("Default Value must not be null");
				}

				IJsonNode result = Get(path);
				if(result == null)
				{
					return defaultV;
				}
				...
				{
					if(defaultV is Boolean) 
					{
						return (T)Convert.ChangeType(result.AsBool(), typeof(T));
					} 
					else if(defaultV is Double) 
					{
						return (T)Convert.ChangeType(result.AsDouble(), typeof(T));
					} 
					else if(defaultV is Int32)
					{
						return (T)Convert.ChangeType(result.AsInt(), typeof(T));
					} 
					else if(defaultV is String) 
					{
						return (T)Convert.ChangeType(result.AsString(), typeof(T));
					} 
					else 
					{
						Console.WriteLine("Unable to wrap the given object " + path);
						return defaultV;
					}
				}
			}
			...
			public void SetDictValue(string key, double value) 
			{
				this[key] = new JsonNodeValueNumber(value);
			}
			...
			public void SetDictValue(string key, string value)
			{
				this[key] = new JsonNodeValueString(value);
			}
